...
5. **可见的复用证明。** 校园演示和项目决策库采用不同节点／关系类型，共享核心、适配器和界面。进一步将核心拆成独立包，并在真实 `xjtlu-unofficial-guide` 的答案详情页用公开投影实现“同主题其他答案”，复用原业务筛选与卡片提示，见[集成记录](integrations/xjtlu-unofficial-guide.md)。
